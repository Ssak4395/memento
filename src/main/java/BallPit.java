...
class BallPit {
    ...
    int min = 30;
    int max = 300;
  ArrayList<Ball> BallClone = new ArrayList<>();
CloneFactory cf = new CloneFactory();

    BallPit(double width, double height) {
        this.height = height;
        this.width = width;
        balls.add(new Ball(100,100,20,Paint.valueOf("BLACK"),new LazyBallStrategy()));
      balls.add(new Ball(100,100,20,Paint.valueOf("RED"),new LazyBallStrategy()));

      balls.add(new Ball(100,100,20,Paint.valueOf("BLUE"),new LazyBallStrategy()));
...
//                    System.out.println("collided");
    ...
    void handlenewBall(String Colour,String strategy)
    {
        if(strategy.equals("LazyBall"))
        {
            int r = ThreadLocalRandom.current().nextInt(min, max + 1);
            balls.add(new Ball(r,r,10,Paint.valueOf(Colour.toUpperCase()),new LazyBallStrategy()));
        }
        if(strategy.equals("StickyCorner"))
        {
            int r = ThreadLocalRandom.current().nextInt(min, max + 1);

            balls.add(new Ball(r,r,10,Paint.valueOf(Colour.toUpperCase()),new StickyCornerStrategy()));

        }
        if(strategy.equals("CollisionFreeze"))
        {
            int r = ThreadLocalRandom.current().nextInt(min, max + 1);
            balls.add(new Ball(r,r,10,Paint.valueOf(Colour.toUpperCase()),new CollisionFreezeStrategy()));
        }
    }
    ...
    {
    ++SaveFiles;
     BallClone = DeepCopy(balls);
      c.addMemento(new Memento(BallClone));
    ...
    public void Revert() {
      ...
      if (SaveFiles == 1) {
        balls.clear();
        System.out.print(SaveFiles);
        balls = c.getMemento(SaveFiles - 1).getBallList();

      }

    }

    public void ClearAll(){balls.clear();}

   public ArrayList<Ball> DeepCopy(ArrayList<Ball> b )
   {
     Iterator<Ball> iterator = b.iterator();

     while(iterator.hasNext())
     {
       //Add the object clones
       BallClone.add((Ball) iterator.next().clone());
     }
     return BallClone;
   }
